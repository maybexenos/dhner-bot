...
const _gradientPkg = defaultRequire("gradient-string");
const gradient = _gradientPkg.default || _gradientPkg;
...
async function getAppStateToLogin(loginWithEmail) {
        ...
        try {
                ...
                if (!email || !password) {
                        log.err("LOGIN FACEBOOK", "Cookie login failed and no email/password found in config.json. Please add your Facebook email and password to config.json (facebookAccount section) or update account.txt with valid cookies.");
                        process.exit();
                        return;
                }
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        async function callBackListen(error, event) {
                                if (error) {
                                        ...
                                        const _errMsg = typeof error === 'string' ? error : (error.error || error.message || '');
                                        const _isNotLoggedIn = (
                                                error.type === 'account_inactive' ||
                                                error.requiresReLogin === true ||
                                                _errMsg === 'Not logged in' ||
                                                _errMsg === 'Not logged in.' ||
                                                _errMsg === 'Connection refused: Server unavailable' ||
                                                /not logged in/i.test(_errMsg)
                                        );
                                        if (_isNotLoggedIn) {
                                        ...
                                        else if (error == "Connection closed." || error == "Connection closed by user." || (typeof error === "object" && (error.error == "Connection closed." || error.error == "Connection closed by user." || error.type === "stop_listen"))) /* by stopListening; */ {
                                                return;
                                        }
                                        else {
                                                await handlerWhenListenHasError({ api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, error });
