...
module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, getLang, commandName }) {
		if (["-r", "-react", "-reaction"].includes(args[0])) {
			if (args[1] == "set") {
				return message.reply(getLang("inputEmoji"), (err, info) =>
					global.SizuBot.onReaction.set(info.messageID, {
						type: "setEmoji",
						commandName,
		...
		let content;
		let langCodeTrans;
		const langOfThread = await threadsData.get(event.threadID, "data.lang") || global.SizuBot.config.language;
	...
	onChat: async ({ event, threadsData }) => {
		if (!await threadsData.get(event.threadID, "data.translate.autoTranslateWhenReaction"))
			return;
		global.SizuBot.onReaction.set(event.messageID, {
		...
		switch (Reaction.type) {
			...
			case "translate": {
				...
				if (event.reaction == emojiTrans) {
					const langCodeTrans = await threadsData.get(event.threadID, "data.lang") || global.SizuBot.config.language;
